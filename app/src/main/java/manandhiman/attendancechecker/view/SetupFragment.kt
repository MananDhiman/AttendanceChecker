...
class SetupFragment : Fragment() {
  ...
  private fun createForm(num: Int) {
    ...
    btn.setOnClickListener {
      if(isListValid(editTextList)) {
        ...
        Toast.makeText(context, "Will relaunch app in 2 seconds. Please wait",Toast.LENGTH_LONG).show()
        activity?.recreate()
  ...
  private fun isListValid(editTextList: ArrayList<EditText>): Boolean {
    ...
    for(i in editTextList) {
      ...
      if(i.text.isEmpty()) {
        ...
        Toast.makeText(context,"A subject name is quite long. Make sure it's correct",Toast.LENGTH_SHORT).show()
        i.requestFocus()
        return false
      }
    }
  ...
  private fun isNumberValid(num: Int): Boolean {
    ...
    if(num < 1) {
      ...
      Toast.makeText(context,"You're entering a large number. Make sure it's correct",Toast.LENGTH_SHORT).show()
      return true
    }
    return true
  }
